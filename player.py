...
class Player(CircleShape):
    def __init__(self, x, y):
        ...
        self.bigshot_cooldown = PLAYER_SHOOT_COOLDOWN * 2  # Slower fire rate for BigShot
    ...
    def shoot(self, dt):
        if self.timer <= 0:
            ...
            if self.weapon_type == "default":
                ...
                shot = FastShot(self.position.x + forward.x * self.radius, self.position.y + forward.y * self.radius)
                shot.velocity = forward * PLAYER_SHOOT_SPEED
                return shot
                ...
                for angle in [-15, 0, 15]:
                    spread_shot = SpreadShot(self.position.x + forward.x * self.radius, self.position.y + forward.y * self.radius, angle)
                    spread_shot.set_velocity(forward * PLAYER_SHOOT_SPEED)
                    spread_shots.append(spread_shot)
                return spread_shots

            elif self.weapon_type == "big":
                shot = BigShot(self.position.x + forward.x * self.radius, self.position.y + forward.y * self.radius)
                shot.velocity = forward * (PLAYER_SHOOT_SPEED * 0.8)  # Slower speed for BigShot
                return shot
